...
class Blockchain:
    def  __init__(self) -> None:
        self.chain = list()
        genesis_block = self._create_block(data='i  am  genesis block', proof=1, previous_hash="0", index=1)
    ...
    def _hash(self, block: dict) -> str:
        """
        Hash  a  block   and return  a  cryptographic hash  of the  block
        """
    ...
    def _to_digest(self, new_proof: int, previous_proof: int,  index: str, data: str) -> bytes:
        to_digest  = str(new_proof ** 2 - previous_proof ** 2 + index) + data #Our formula to calculate proof
        
        return  to_digest.encode()  # returns  utf8 encoding 
    ...
    def _proof_of_work(self, previous_proof: str, index: int, data: str) -> int:
        ...
        while not check_proof:
            ...
            if hash_value[:4] == "0000": # computation  time for proof of  work
                check_proof = True
            else:
                new_proof += 1
